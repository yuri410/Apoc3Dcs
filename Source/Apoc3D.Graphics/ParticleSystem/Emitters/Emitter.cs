...
namespace Apoc3D.Graphics.ParticleSystem.Emitters
{
    ...
    public class Emitter<T> where T : IParticle, new()
    {
        ...
        public Emitter(int budget, float term)
        {
            ...
            this._pool = new ParticlePool<T>(budget);
        }
        ...
        //{
        //    this._pool = new ParticlePool<T>(this._budget);
        ...
        [NonSerialized]
        private ParticlePool<T> _pool;                              //Particle pool.
        ...
        [NonSerialized]
        private Texture _texture;                                 //Reference to a texture used to draw particles.
        ...
        internal void Update(float totalTime, float elapsedTime)
        {
            ...
            while (this._triggers.Count > 0)
            {
                ...
                for (int i = 0; i < this._quantity; i++)
                {
                    if (this._pool.AvailableCount > 0)
                    {
                        ...
                        IParticle particle =  this._pool.Get(index);
                        ...
                        for (int ii = 0; ii < this._modifiers.Count; ii++)
                            this._modifiers[ii].ProcessReleasedParticle(totalTime, particle);
            ...
            {
                IParticle particle = this._pool.Get(i);
                ...
                if (age < 1.0f)
                {
                    ...
                    //Send the particle to the modifiers...
                    for (int ii = 0; ii < this._modifiers.Count; ii++)
                        this._modifiers[ii].ProcessActiveParticle(totalTime, elapsedTime, particle,age);
                }
                else
                {
                    //Send the particle to the modifiers...
                    for (int ii = 0; ii < this._modifiers.Count; ii++)
                        this._modifiers[ii].ProcessRetiredParticle(totalTime, particle);
        ...
        {
            float rads = (float)_rnd.NextDouble() * MathEx.TwoPi;
