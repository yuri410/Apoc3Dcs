...
namespace VirtualBicycle.Graphics.ParticleSystem.Modifiers
{
    ...
    public class RandomOpacityModifier : Modifier
    {
        ...
        public override void ProcessReleasedParticle(float totalTime, IParticle particle)
        {
            float randOpacity = this._weight + (((float)Random.NextDouble() * this._variation) - (this._variation / 2f));

            particle.Opacity = MathEx.Clamp(randOpacity, 0f, 1f);
        }
    }
}
