    ...
    public class ConeShape : ConvexShape
    {
        ...
        private Vector3 ConeLocalSupport(Vector3 v)
        {
            ...
            if (condition)
            {
                ...
                MathUtil.SetValueByIndex(ref tmp, _coneIndices[1], halfHeight);
                return tmp;
            }
            else
            {
                float s = (float)Math.Sqrt(MathUtil.GetValueByIndex(v, _coneIndices[0]) * MathUtil.GetValueByIndex(v, _coneIndices[0])
                    + MathUtil.GetValueByIndex(v, _coneIndices[2]) * MathUtil.GetValueByIndex(v, _coneIndices[2]));
                if (s > MathUtil.Epsilon)
                {
                    float d = _radius / s;
                    Vector3 tmp = new Vector3();
                    MathUtil.SetValueByIndex(ref tmp, _coneIndices[0], MathUtil.GetValueByIndex(v, _coneIndices[0]) * d);
                    MathUtil.SetValueByIndex(ref tmp, _coneIndices[1], -halfHeight);
                    MathUtil.SetValueByIndex(ref tmp, _coneIndices[2], MathUtil.GetValueByIndex(v, _coneIndices[2]) * d);
                    return tmp;
                }
                else
                {
