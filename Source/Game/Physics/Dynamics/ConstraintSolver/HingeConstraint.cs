...
namespace VirtualBicycle.Physics.Dynamics
{
    ...
    public class HingeConstraint : TypedConstraint
    {
        ...
        {
            _pivotInA = pivotInA;
            _pivotInB = MathUtil.MatrixToVector(rbA.CenterOfMassTransform, pivotInA);
            _axisInA = axisInA;
            //fixed axis in worldspace
            _axisInB = MathUtil.TransformNormal(-axisInA, rbA.CenterOfMassTransform);
            _angularOnly = false;
        }
        ...
        public override void BuildJacobian()
        {
            ...
            if (!_angularOnly)
            {
                for (int i = 0; i < 3; i++)
                {
                    MathUtil.SetElement(ref normal, i, 1);
                    _jac[i] = new JacobianEntry(
                            ...
                            MathUtil.Transform(_pivotInA, RigidBodyA.CenterOfMassTransform) - RigidBodyA.CenterOfMassPosition,
                            MathUtil.Transform(_pivotInB, RigidBodyB.CenterOfMassTransform) - RigidBodyB.CenterOfMassPosition,
                            normal,
                            RigidBodyA.InvInertiaDiagLocal,
                            RigidBodyA.InverseMass,
                            RigidBodyB.InvInertiaDiagLocal,
                            RigidBodyB.InverseMass);
                    MathUtil.SetElement(ref normal, i, 0);
                }
            }
            ...
            MathUtil.PlaneSpace1(_axisInA, ref jointAxisALocal, ref jointAxisBLocal);

            Vector3 jointAxisA = MathUtil.TransformNormal(jointAxisALocal, RigidBodyA.CenterOfMassTransform);
            Vector3 jointAxisB = MathUtil.TransformNormal(jointAxisBLocal, RigidBodyA.CenterOfMassTransform);
            Vector3 hingeAxisWorld = MathUtil.TransformNormal(_axisInA, RigidBodyA.CenterOfMassTransform);
        ...
        {
            Vector3 pivotAInW = MathUtil.Transform(_pivotInA, RigidBodyA.CenterOfMassTransform);
            Vector3 pivotBInW = MathUtil.Transform(_pivotInB, RigidBodyB.CenterOfMassTransform);
            ...
            Vector3 axisA = MathUtil.TransformNormal(_axisInA, RigidBodyA.CenterOfMassTransform);
            Vector3 axisB = MathUtil.TransformNormal(_axisInB, RigidBodyB.CenterOfMassTransform);
