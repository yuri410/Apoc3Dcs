...
#pragma warning disable 
    ...
    public class SequentialImpulseConstraintSolver : IConstraintSolver
    {
        ...
        {
            get { return _solverMode; }
            set { _solverMode = value; }
        }
        public static int SequentialImpulseMaxSolverPoints
        {
            get { return _sequentialImpulseMaxSolverPoints; }
        }
            ...
            //only necessary to refresh the manifold once (first iteration). The integration is done outside the loop
            {
                ...
                for (int i = 0; i < numpoints; i++)
                {
                    ...
                    if (cp.Distance <= 0)
                    {
                        ...
                        MathUtil.PlaneSpace1(cp.NormalWorldOnB, ref fwta, ref fwtb);
            ...
            float appliedNormalImpulse)
        {
            ...
            //friction
            {
                float j1;
                {
                    ...
                    MathUtil.SetMin(ref test, limit);
                    MathUtil.SetMax(ref test, -limit);
        ...
        public virtual float SolveGroupCacheFriendly(List<CollisionObject> bodies, List<PersistentManifold> manifolds, int numManifolds, List<TypedConstraint> constraints, ContactSolverInfo infoGlobal, IDebugDraw debugDrawer)
        {
            ...
            for (int i = 0; i < bodies.Count; i++)
            {
                ...
                {
                    PhysDebug.Assert(rb.CompanionID < 0);
...
#warning Check to see if we need Vector3.Transform
                        ...
                        MathUtil.PlaneSpace1(cp.NormalWorldOnB, ref frictionTangential0a, ref frictionTangential1b);
                        {
                            SolverConstraint solverConstraint = new SolverConstraint();
        ...
        protected struct OrderIndex
        {
            ...
            {
                get { return _manifoldIndex; }
                set { _manifoldIndex = value; }
            }
