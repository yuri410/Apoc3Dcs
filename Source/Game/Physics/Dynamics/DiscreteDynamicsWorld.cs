    ...
    public class DiscreteDynamicsWorld : DynamicsWorld
    {
        ...
        public void DebugDrawObject(Matrix worldTransform, CollisionShape shape, Vector3 color)
        {
            // Draw a small simplex at the center of the object
            {
                ...
                DebugDrawer.DrawLine(start, start + MathUtil.TransformNormal(Vector3.UnitX, worldTransform), Vector3.UnitX);
                DebugDrawer.DrawLine(start, start + MathUtil.TransformNormal(Vector3.UnitY, worldTransform), Vector3.UnitY);
                DebugDrawer.DrawLine(start, start + MathUtil.TransformNormal(Vector3.UnitZ, worldTransform), Vector3.UnitZ);
            }
                    ...
                    case BroadphaseNativeTypes.Cylinder:
                        {
                            ...
                            float halfHeight = MathUtil.GetElement(cylinder.HalfExtents, upAxis);
                            Vector3 start = worldTransform.Translation;
                            Vector3 offsetHeight = new Vector3();
                            MathUtil.SetElement(ref offsetHeight, upAxis, halfHeight);
                            Vector3 offsetRadius = new Vector3();
                            MathUtil.SetElement(ref offsetRadius, (upAxis + 1) % 3, radius);
        ...
        public static void DrawAabb(IDebugDraw debugDrawer, Vector3 from, Vector3 to, Vector3 color)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    int othercoord = j % 3;
                    MathUtil.SetElement(ref edgecoord, othercoord, MathUtil.GetElement(edgecoord, othercoord) * -1f);
                ...
                if (i < 3)
                    MathUtil.SetElement(ref edgecoord, i, MathUtil.GetElement(edgecoord, i) * -1f);
            }
        }
        ...
        protected void SynchronizeMotionStates()
        {
            //debug vehicle wheels
            {
                ...
                for (int i = 0; i < CollisionObjects.Count; i++)
                {
                    ...
                    {
                        Vector3 color = new Vector3(255f);

                        switch (colObj.ActivationState)
                        {
                            case ActivationState.Active:
                                color = new Vector3(255f); break;
                ...
                for (int i = 0; i < _vehicles.Count; i++)
                {
                    for (int v = 0; v < _vehicles[i].GetNumWheels(); v++)
                    {
                        ...
                        Vector3 axle = new Vector3(
                            MathUtil.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 0, _vehicles[i].RightAxis),
                            MathUtil.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 1, _vehicles[i].RightAxis),
                            MathUtil.GetElement(_vehicles[i].GetWheelInfo(v).WorldTransform, 2, _vehicles[i].RightAxis));
