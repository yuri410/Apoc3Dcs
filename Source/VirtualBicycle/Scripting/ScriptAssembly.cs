...
namespace VirtualBicycle.Scripting
{
    ...
    public sealed class ScriptAssembly : ExternalModule, IDisposable
    {
        ...
        //    try
        //    {
        //        FileSystem.Instance.Locate(ScriptDll, FileLocateRules.Root);
        //        return true;
        //    }
        ...
        protected override void Load()
        {
            ...
            EngineConsole.Instance.Write("ResourceAssembly.Instance.CM_ScriptAssemblyLoaded(assembly.FullName)", ConsoleMessageType.Information);
            EngineConsole.Instance.Write("ResourceAssembly.Instance.CM_ClassCount(cCount)", ConsoleMessageType.Normal);
            EngineConsole.Instance.Write("ResourceAssembly.Instance.CM_TypeCount(types.Count)", ConsoleMessageType.Normal);
        }

        public ScriptAssembly(string dll, string @namespace)
            : base(FileSystem.Instance.Locate(dll, FileLocateRules.Default), true)
        {
            this.ScriptDll = dll;
            this.ScriptNamespace = @namespace;

            ServiceHelper.CurrentAssembly = this;
        }
        public ScriptAssembly(FileLocation fl, string @namespace)
            : base(fl, true) 
        {
            this.ScriptDll = Path.GetFileName(fl.Path);
            this.ScriptNamespace = @namespace;
