...
namespace VirtualBicycle.CollisionModel.Shapes
{
    ...
    public abstract class ConvexShape : CollisionShape
    {
        ...
        public static int MaxPreferredPenetrationDirections
        {
            get { return _maxPreferredPenetrationDirections; }
        }
        public static float ConvexDistanceMargin
        {
            get { return _convexDistanceMargin; }
        }

        public Vector3 ImplicitShapeDimensions
        {
            get { return _implicitShapeDimensions; }
            protected set { _implicitShapeDimensions = value; }
        }
        public virtual int PreferredPenetrationDirectionsCount
        {
            get { return 0; }
        }
        ...
        public virtual void GetAabbSlow(Matrix t, out Vector3 aabbMin, out Vector3 aabbMax)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                MathUtil.SetElement(ref vec, i, 1);

                Vector3 sv = LocalGetSupportingVertex(Vector3.TransformNormal(vec, t));

                Vector3 tmp = MathUtil.MatrixToVector(t, sv);
                MathUtil.SetElement(ref aabbMax, i, MathUtil.GetElement(tmp, i) + margin);
                MathUtil.SetElement(ref vec, i, -1f);
                tmp = MathUtil.MatrixToVector(t, LocalGetSupportingVertex(Vector3.TransformNormal(vec, t)));
                MathUtil.SetElement(ref aabbMin, i, MathUtil.GetElement(tmp, i) - margin);
            }
        }
    }
}
