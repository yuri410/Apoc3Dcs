...
namespace VirtualBicycle.CollisionModel.Dispatch
{
    ...
    public class ManifoldResult : DiscreteCollisionDetectorInterface.Result
    {
        ...
        public static ContactAddedCallback ContactAddedCallback
        {
            get { return _contactAddedCallback; }
            set { _contactAddedCallback = value; }
        }
        ...
        {
            if (_manifold == null)
                throw new PhysicsException("Manifold Pointer is null.");
            ...
            if (isSwapped)
            {
                localA = MathUtil.InvXForm(_rootTransB, pointA);
                localB = MathUtil.InvXForm(_rootTransA, pointInWorld);
            }
            else
            {
                localA = MathUtil.InvXForm(_rootTransA, pointA);
                localB = MathUtil.InvXForm(_rootTransB, pointInWorld);
            }
