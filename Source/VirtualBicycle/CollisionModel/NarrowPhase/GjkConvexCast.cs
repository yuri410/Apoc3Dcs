...
namespace VirtualBicycle.CollisionModel.NarrowPhase
{
    ...
    public class GjkConvexCast : IConvexCast
    {
        ...
        public bool CalcTimeOfImpact(Matrix fromA, Matrix toA, Matrix fromB, Matrix toB, CastResult result)
        {
            ...
            Matrix rayFromLocalA = MathUtil.InvertMatrix(fromA) * fromB;
            Matrix rayToLocalA = MathUtil.InvertMatrix(toA) * toB;
            ...
            if (hasResult)
            {
                ...
                while (dist > radius)
                {
                    ...
                    if (dot >= -(MathUtil.Epsilon * MathUtil.Epsilon)) return false;
