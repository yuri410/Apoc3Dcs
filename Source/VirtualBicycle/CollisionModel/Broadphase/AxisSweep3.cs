...
namespace VirtualBicycle.CollisionModel.Broadphase
{
    ...
    public class AxisSweep3 : OverlappingPairCache
    {
        ...
        public AxisSweep3(Vector3 worldAabbMin, Vector3 worldAabbMax, int maxHandles)
            : base()
        {
            PhysDebug.Assert(maxHandles > 1 && maxHandles < 32767);
        ...
        public override void ProcessAllOverlappingPairs(IOverlapCallback callback)
        {
            ...
            for (int i = 0; i < OverlappingPairs.Count; i++)
            {
                ...
                {
                    needsRemoval = true;
                    PhysDebug.Assert(previousPair.CollisionAlgorithm == null);
                }
        ...
        {
            PhysDebug.Assert(handle > 0 && handle < _maxHandles);
        ...
        private void Quantize(out ushort[] result, Vector3 point, int isMax)
        {
            ...
            MathUtil.SetMax(ref clampedPoint, _worldAabbMin);
            MathUtil.SetMin(ref clampedPoint, _worldAabbMax);
