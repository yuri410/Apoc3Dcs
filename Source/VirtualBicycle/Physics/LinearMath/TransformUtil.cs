...
namespace VirtualBicycle.Physics
{
	...
	{
		const float AngularMotionTreshold = 0.5f * VirtualBicycle.Physics.MathLib.MathHelper.PiOver2;
		...
		public static void CalculateVelocity(Matrix transformA, Matrix transformB, float timeStep, ref Vector3 linearVelocity, ref Vector3 angularVelocity)
		{
            ...
            Matrix dmat = transformB * MathUtil.InvertMatrix(transformA);
			...
			if (len < MathUtil.Epsilon * MathUtil.Epsilon)
		...
		{
			Matrix dmat = transformB * MathUtil.InvertMatrix(transformA);
			Quaternion dorn = MathUtil.GetRotation(dmat);
