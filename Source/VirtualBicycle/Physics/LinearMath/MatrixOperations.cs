	...
	internal static class MatrixOperations
	{
		...
		public static Quaternion GetRotation(Matrix m)
		{
			...
			else
			{
				...
				float s = (float)Math.Sqrt(MathUtil.GetElement(m, i, i) - MathUtil.GetElement(m, j, j) - MathUtil.GetElement(m, k, k) + 1);
				MathUtil.SetElement(ref q, i, s * 0.5f);
				s = 0.5f / s;

				q.W = (MathUtil.GetElement(m, k, j) - MathUtil.GetElement(m, j, k)) * s;
				MathUtil.SetElement(ref q, j, (MathUtil.GetElement(m, j, i) + MathUtil.GetElement(m, i, j)) * s);
				MathUtil.SetElement(ref q, k, (MathUtil.GetElement(m, k, i) + MathUtil.GetElement(m, i, k)) * s);
			}
		...
		public static Matrix Multiply(Matrix a, Matrix b)
		{
			...
			return new Matrix(
				Dot(b, 0, MathUtil.GetRow(a, 1)), Dot(b, 1, MathUtil.GetRow(a, 1)), Dot(b, 2, MathUtil.GetRow(a, 1)), 0,
				Dot(b, 0, MathUtil.GetRow(a, 2)), Dot(b, 1, MathUtil.GetRow(a, 2)), Dot(b, 2, MathUtil.GetRow(a, 2)), 0,
				Dot(b, 0, MathUtil.GetRow(a, 3)), Dot(b, 1, MathUtil.GetRow(a, 3)), Dot(b, 2, MathUtil.GetRow(a, 3)), 0,
				0, 0, 0, 1);
		}

		public static float Dot(Matrix m, int c, Vector3 v) 
		{
			return MathUtil.GetElement(m, 0, c) * v.X + MathUtil.GetElement(m, 1, c) * v.Y + MathUtil.GetElement(m, 2, c) * v.Z;
		}
