    ...
    public class Generic6DofConstraint : TypedConstraint
    {
        ...
        public float ComputeAngle(int axis)
        {
            ...
            switch (axis)
            {
                case 0:
                    {
                        Vector3 v1 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInA, 1), RigidBodyA.CenterOfMassTransform);
                        Vector3 v2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 1), RigidBodyB.CenterOfMassTransform);
                        Vector3 w2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 2), RigidBodyB.CenterOfMassTransform);
                ...
                case 1:
                    {
                        Vector3 w1 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInA, 2), RigidBodyA.CenterOfMassTransform);
                        Vector3 w2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 2), RigidBodyB.CenterOfMassTransform);
                        Vector3 u2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 0), RigidBodyB.CenterOfMassTransform);
                ...
                case 2:
                    {
                        Vector3 u1 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInA, 0), RigidBodyA.CenterOfMassTransform);
                        Vector3 u2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 0), RigidBodyB.CenterOfMassTransform);
                        Vector3 v2 = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, 1), RigidBodyB.CenterOfMassTransform);
                ...
                default: PhysDebug.Assert(false); break;
            }
        ...
        public override void BuildJacobian()
        {
            ...
            Vector3 pivotAInW = MathUtil.Transform(_frameInA.Translation, RigidBodyA.CenterOfMassTransform);
            Vector3 pivotBInW = MathUtil.Transform(_frameInB.Translation, RigidBodyB.CenterOfMassTransform);
            ...
            for (int i = 0; i < 3; i++)
            {
                if (IsLimited(i))
                {
                    ...
                    Vector3 normalWorld = MathUtil.TransformNormal(localNormalInA, RigidBodyA.CenterOfMassTransform);
                    ...
                    _jacLinear[i] = new JacobianEntry(
                        ...
                        MathUtil.Transform(pivotInA, RigidBodyA.CenterOfMassTransform) - RigidBodyA.CenterOfMassPosition,
                        MathUtil.Transform(pivotInB, RigidBodyB.CenterOfMassTransform) - RigidBodyB.CenterOfMassPosition,
                        normalWorld,
            ...
            for (int i = 0; i < 3; i++)
            {
                if (IsLimited(i + 3))
                {
                    Vector3 axisA = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInA, _axisA[i] + 1), RigidBodyA.CenterOfMassTransform);
                    Vector3 axisB = MathUtil.TransformNormal(MathUtil.GetColumn(_frameInB, _axisB[i] + 1), RigidBodyB.CenterOfMassTransform);
        ...
        public override void SolveConstraint(float timeStep)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                if (IsLimited(i))
                {
                    Vector3 angvelA = MathUtil.TransformNormal(RigidBodyA.AngularVelocity, MatrixOperations.Transpose(RigidBodyA.CenterOfMassTransform));
                    Vector3 angvelB = MathUtil.TransformNormal(RigidBodyB.AngularVelocity, MatrixOperations.Transpose(RigidBodyB.CenterOfMassTransform));
        ...
        private bool MatrixToEulerXYZ(Matrix mat, out Vector3 xyz)
        {
            ...
            if (MathUtil.GetElement(mat, 2) < 1.0f)
            {
                if (MathUtil.GetElement(mat, 2) > -1.0f)
                {
                    xyz.X = (float)Math.Atan2(-MathUtil.GetElement(mat, 5), MathUtil.GetElement(mat, 8));
                    xyz.Y = (float)Math.Asin(MathUtil.GetElement(mat, 2));
                    xyz.Z = (float)Math.Atan2(-MathUtil.GetElement(mat, 1), MathUtil.GetElement(mat, 0));
                    return true;
                }
                else
                {
                    // WARNING.  Not unique.  XA - ZA = -atan2(r10,r11)
                    xyz.X = -(float)Math.Atan2(MathUtil.GetElement(mat, 3), MathUtil.GetElement(mat, 4));
