 ...
 *
 *
 * COMPONENT:           $RCSfile: AppHardwareApi.h,v $
 *
 * VERSION:             $Name: RD_RELEASE_6thMay09 $
 *
 * REVISION:            $Revision: 1.20 $
 *
 * DATED:               $Date: 2009/01/05 13:32:38 $
 *
 * STATUS:              $State: Exp $
 *
 * AUTHOR:              CJG
...
PUBLIC void   vAHI_BrownOutConfigure(uint8 const u8VboSelect,
									bool_t const bVboRestEn,
									bool_t const bVboEn );
/* Ref[PR202] */
PUBLIC bool_t vAHI_BrownOutDetection(void);
...
PUBLIC uint16 AHI_AdcAccumulatedRead(void);
...
PUBLIC void   vAHI_UartRegisterCallback(uint8 u8Uart, PR_HWINT_APPCALLBACK prUart1Callback);
PUBLIC PR_HWINT_APPCALLBACK prAHI_UartReadCallback(uint8 u8Uart);
...
PUBLIC void  vAHI_TimerDIOControl(uint8 u8Timer, bool_t bDIOEnable);
